...
Give reviewers the known task requirements, existing contracts the change must preserve, applicable written
rules, and the load-bearing claim. Ask them to falsify that claim and name what each finding violates. A finding
earns a code change only when it demonstrates that the change violates a task requirement, an existing contract
it must preserve, or an applicable written rule. If it only falsifies the claim, correct the claim instead.
Otherwise it proposes new scope; ask the user before implementing it. Re-review the resulting diff using this
eligibility rule. Stop when no qualifying finding remains.

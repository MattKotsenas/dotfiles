...
public sealed class IntentDispatchRule : IEventRule
{
    ...
    private readonly Dictionary<string, Func<Command>> _intentMap;

    public string Name => "IntentDispatchRule";

    public IntentDispatchRule(ILogger<IntentDispatchRule> logger, ICommandRunner runner)
    {
        _logger = logger;
        _runner = runner;
    ...
    public void ProcessEvent(IEvent evt)
    {
        ...
        if (!_intentMap.TryGetValue(intent, out var commandFactory))
        {
    ...
    private static Dictionary<string, Func<Command>> BuildIntentMap()
    {
